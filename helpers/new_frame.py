...
def euler_in_new_frame(euler_old_frame, euler_frame_old_2_new, rot_seq, is_degree):
  ...
  logger.info('body rotation in old frame: euler(%s)%s' % (util.get_angular_unit(is_degree), rot_old_frame.as_euler(rot_seq, is_degree))) # call as_euler() to get normalized euler

  rot_frame_old_2_new = Rotation.from_euler(rot_seq, euler_frame_old_2_new, is_degree)
  logger.info('frame rotation from old to new: euler(%s)%s' % (util.get_angular_unit(is_degree), rot_frame_old_2_new.as_euler(rot_seq, is_degree)))

  rot_new_frame = rot_in_new_frame(rot_old_frame, rot_frame_old_2_new)
  euler_new_frame = rot_new_frame.as_euler(rot_seq, is_degree)
  logger.info('body rotation in new frame: euler(%s)%s' % (util.get_angular_unit(is_degree), euler_new_frame))
